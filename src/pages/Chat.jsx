...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    socket.current = io('https://5.199.169.195:5000', {
      auth: {
        token
      },
  ...
  useEffect(() => {
    ...
    const fetchChats = async () => {
      try {
        const response = await axios.get('https://5.199.169.195:5000/api/chat');
        const chatsWithLast = await Promise.all(response.data.map(async (chat) => {
          try {
            const msgRes = await axios.get(`https://5.199.169.195:5000/api/chat/${chat.id}/messages`);
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      if (selectedChat) {
        try {
          const response = await axios.get(`https://5.199.169.195:5000/api/chat/${selectedChat.id}/messages`);
    ...
    const fetchMembers = async () => {
      if (selectedChat && selectedChat.type !== 'private') {
        try {
          const response = await axios.get(`https://5.199.169.195:5000/api/group/${selectedChat.id}/members`, {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await axios.get('https://5.199.169.195:5000/api/users', {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
        });
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      await axios.post(
        `https://5.199.169.195:5000/api/chat/${selectedChat.id}/messages`,
        { content: newMessage }
      );
  ...
  const handleCreatePrivateChat = async (userId) => {
    try {
      const response = await axios.post('https://5.199.169.195:5000/api/chat/private', 
        { userId },
        { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }
      );
  ...
  const handleDeleteChat = async () => {
    ...
    try {
      await axios.delete(`https://5.199.169.195:5000/api/chat/${selectedChat.id}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
          ...
          <div className="w-64 border-l border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 p-4 overflow-y-auto">
                    ...
                    {myRole && member.id !== user.id && member.role !== 'owner' && (
                      <>
                        {myRole === 'owner' && member.role === 'admin' && (
                          <button onClick={async () => { await axios.patch(`https://5.199.169.195:5000/api/group/${selectedChat.id}/members/${member.id}/role`, { role: 'member' }); window.location.reload(); }} className="text-xs text-yellow-600 ml-2">Pašalinti admin</button>
                        )}
                        {(myRole === 'owner' || (myRole === 'admin' && member.role === 'member')) && member.role === 'member' && (
                          <button onClick={async () => { await axios.patch(`https://5.199.169.195:5000/api/group/${selectedChat.id}/members/${member.id}/role`, { role: 'admin' }); window.location.reload(); }} className="text-xs text-green-600 ml-2">Padaryti admin</button>
                        )}
                        {(myRole === 'owner' || (myRole === 'admin' && member.role === 'member')) && (
                          <button onClick={async () => { await axios.delete(`https://5.199.169.195:5000/api/group/${selectedChat.id}/members/${member.id}`); window.location.reload(); }} className="text-xs text-red-600 ml-2">Pašalinti</button>
                        )}
                      </>
                    )}
                  </li>
            ...
            {canLeave && (
              <button
                ...
                onClick={async () => {
                  await axios.delete(`https://5.199.169.195:5000/api/group/${selectedChat.id}/leave`);
            ...
            {myRole === 'owner' && selectedChat && selectedChat.type !== 'private' && (
              <button
                ...
                onClick={async () => {
                  await axios.delete(`https://5.199.169.195:5000/api/group/${selectedChat.id}`);
                  window.location.reload();
                }}
              >
                Ištrinti grupę
