...
export default function AddMemberModal({ chatId, onClose, onAdded }) {
  ...
  useEffect(() => {
    ...
    axios.get(`https://5.199.169.195:5000/api/group/${chatId}/members`)
      .then(res => setMembers(res.data.map(m => m.id)));
    // Fetch all users
    axios.get(`https://5.199.169.195:5000/api/users`)
  ...
  const handleAdd = async () => {
    for (const userId of selected) {
      await axios.post(`https://5.199.169.195:5000/api/group/${chatId}/members`, { userId });
    }
    onAdded();
    onClose();
  };
